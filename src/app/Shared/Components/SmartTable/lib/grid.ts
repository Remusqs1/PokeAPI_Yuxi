...
export class Grid {
  ...
  constructor(source: DataSource, settings: any, totalRecords: number, noRecordsMessage: string) {
    this.setSettings(settings);
    this.setSource(source);
    this.setTotalRecords(totalRecords);
    this.setNoRecordsMessage(noRecordsMessage);
  }
  ...
  setNoRecordsMessage(noRecordsMessage: string) {
  ...
  copy(row: Row, confirmEmitter: EventEmitter<any>) {

    const deferred = new Deferred();

    if (this.getSetting('copy.confirmCopy')) {
      confirmEmitter.emit({
        data: row.getData(),
        source: this.source,
        confirm: deferred,
  ...
  processDataChange(changes: any) {
    if (this.shouldProcessChange(changes)) {
      ...
      } else if (this.getSetting('selectMode') === 'multi') {
        this.selectAllRows(this._selectedAll);
      }
  ...
  checkSelectAllRows(status: boolean) {
    this._selectedAll = status;
  }
