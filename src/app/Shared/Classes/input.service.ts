...
export class InputService {
  ...
  applyMask(isNumber: boolean, rawValue: string): string {
    ...
    let integerMax = this.options.integerMax;
    ...
    const isZero = Number(integerPart) === 0 && (Number(decimalPart) === 0 || decimalPart === '');
  ...
  fixCursorPosition(forceToEndPosition?: boolean): void {
    ...
    // if the current cursor position is after the number end position, it is moved to the end of the number,
    // ignoring the prefix or suffix. this behavior can be forced with forceToEndPosition flag
    ...
    // if the current cursor position is before the number start position, it is moved to the start of the number,
    // ignoring the prefix or suffix
  ...
  removeNumber(keyCode: number): void {
    ...
    if (selectionEnd === selectionStart) {
      ...
      if (keyCode === 8 && (this.rawValue.substring(selectionStart - 1, selectionEnd) === decimal
