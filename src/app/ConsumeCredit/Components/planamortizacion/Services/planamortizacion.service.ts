...
export class PlanamortizacionService {
  ...
  getUserProductsGc(input) {
    return this.proxy.executePost('consumeCredit/GetNumberAllCredit', input).pipe(map((response) => {
  ...
  GetFilePeaceSafe(input) {
    return this.proxy.executePost('consumeCredit/GetFilePaymentPlan', input).pipe(map((response) => {
