...
@Injectable()
export class AssociatedFormsService {

    constructor(private formBuilder: FormBuilder, private statusPipe: StatusPipe) { }

    getFilterForm(): FormGroup {
        return this.formBuilder.group({
            allyName: [{ value: undefined, disabled: false }, []],
            allyState: [{ value: undefined, disabled: false }, []],
        });
    }

    getOtpForwardingForm(): FormGroup {
        const form = this.formBuilder.group({
            create_fotp_Email: [{ value: undefined, disabled: false },
            [Validators.required, Validators.email, Validators.minLength(5), CustomValidators.MaxLength(50), Validators.pattern('[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,3}$')]],
            create_fotp_Cellphone: [{ value: undefined, disabled: false },
            [Validators.required, Validators.minLength(10) , CustomValidators.MaxLength(10), CustomValidators.Numeric]],
            create_fotp_IdentificationType: [{ value: undefined, disabled: false },
            [Validators.required, CustomValidators.MaxLength(2), CustomValidators.Numeric]],
            create_ftop_Identification: [{ value: undefined, disabled: false },
            [Validators.required, Validators.min(6), Validators.maxLength(12), Validators.nullValidator, CustomValidators.AlphaNumericWithWhiteSpace, CustomValidators.IsNullorEmpty]]
        });
        return form;
    }
    ...
    getConfigDataTable() {
        ...
        const columns: any = {
            agr_name: {
                title: 'ally.grid.columnsName.name',
                filter: false,
            },
            agr_status: {
                title: 'ally.grid.columnsName.state',
                filter: false,
